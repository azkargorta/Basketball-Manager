...
function clubProfile(c){const rep=c?.reputation||55,health=c?.financialHealth??65;if(rep>=88)return{id:'ELITE',name:'Élite',target:3,youth:1};if(rep>=80)return{id:'CONTENDER',name:'Candidato',target:5,youth:1};if(rep>=70)return{id:'PLAYOFF',name:'Aspirante a playoff',target:8,youth:1};if(rep>=61)return{id:'STABLE',name:'Zona media',target:10,youth:1};if(rep>=52||health>=60)return{id:'DEVELOP',name:'Desarrollo',target:12,youth:2};return{id:'SURVIVAL',name:'Permanencia',target:14,youth:2}}
function ensureClubProjects(){for(const c of state.world?.clubs||[]){const base=clubProfile(c),p=c.careerProject;if(!p||p.modelVersion!==2)c.careerProject={modelVersion:2,profile:base.id,name:base.name,target:base.target,youthTarget:base.youth,season:state.season,history:p?.history||[]}}}
...
function evolveAllClubProjects(){ensureClubProjects();for(const competition of state.world?.competitions||[]){if(!competition.standings)continue;const rows=sortedStandings(competition.id);rows.forEach((row,index)=>evolveClubProject(club(row.clubId),index+1,rows.length))}}
function projectObjectives(clubObj){const p=clubObj?.careerProject||clubProfile(clubObj);return[{id:'LEAGUE',label:'Finalizar entre los '+p.target+' primeros de liga',target:p.target},{id:'YOUTH',label:'Desarrollar talento joven',target:p.youthTarget||p.youth||1},{id:'FIN',label:'Mantener una estructura financiera sostenible',target:1}]}
