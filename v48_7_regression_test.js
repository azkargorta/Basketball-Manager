...
if(!version.includes("label:'v0.48.10 Beta'")||!html.includes('js/app.js?v=v04810')||!sw.includes('basketball-gm-beta-v04810'))throw new Error('Versión o caché incorrectas');
if(!html.includes("serviceWorker.register('./sw.js?v=v04810', { updateViaCache: 'none' })")||!html.includes("addEventListener('controllerchange'"))throw new Error('La app instalada no activa y recarga la caché v0.48.10');
...
if(!app.includes('if(!isCloseMatchDecisionV4810(live))return false'))throw new Error('La decisión puede aparecer con un marcador amplio');
...
console.log(JSON.stringify({decisionScoreUsesExistingTeamRating:true,closeGamesOnly:true,mobileDecisionLayout:true,decisionCompletesMatch:true,legacyDecisionRecovery:true,friendliesBeforeOfficialOpener:true,legacyDateDoesNotGoBackwards:true,networkFirstCache:true,version:'v0.48.10',ok:true},null,2));
