...
def read_page(label,url):
    r=requests.get(READER+url,headers=HEADERS,timeout=45)
    r.raise_for_status()
    return label,url,r.text
...
def parse_table(text,label):
    ...
    for raw in text.splitlines():
        ...
        if label=='NBA':
            ...
            if name and team:out.append((team,{"name":name,"pos":pos,"age":age}))
        else:
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=4) as pool:
        futures=[pool.submit(read_page,label,url) for label,url in SOURCES]
        for f in as_completed(futures):
            try:pages.append(f.result())
            except Exception as e:errors.append(f"Reader: {type(e).__name__}: {e}")
    for label,url,text in pages:
        try:
            rows=parse_table(text,label);matched=0
            for team,p in rows:
                cid=team_id(team)
        ...
        except Exception as e:errors.append(f"{label}: {type(e).__name__}: {e}");print('ERROR',label,e,flush=True)
