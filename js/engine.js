(function(g){
  ...
  function metrics(club,rot){
    const off=weightedMetric(club,rot,['finishing','midRange','threePoint','ballHandling','passing','shotCreation','offBall','basketballIq','decisionMaking'])*.88+club.coach.offense*.10+(club.coach.manManagement||65)*.02;
    const def=weightedMetric(club,rot,['perimeterDefense','interiorDefense','helpDefense','steal','block','defensiveRebound','basketballIq','strength'])*.88+club.coach.defense*.10+(club.coach.manManagement||65)*.02;
  ...
  function playPossession(offense,defense,orot,drot,ostats,dstats,om,dm,homeBonus,rng){
    const qd=om.offense-dm.defense;let toProb=clamp(.135+(dm.steals-om.handling)*.001+(defense.style.pressure-50)*.0008-qd*.00025,.08,.22);
  ...
  function simulateMatch(home,away,seed=Date.now()){
    ...
    let pace=clamp(72+(((home.style.pace+away.style.pace)/2)-50)*.22+rng.gaussian()*2.3,64,82),poss=Math.round(pace),h=0,a=0;
