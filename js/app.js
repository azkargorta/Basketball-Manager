...
function ensureClubProjects(){for(const c of state.world?.clubs||[]){if(!c.careerProject){const rep=c.reputation||65;c.careerProject={target:rep>=86?3:rep>=76?6:rep>=66?9:12,level:rep>=82?'Competir por títulos':rep>=70?'Aspirar a playoff':'Construir y crecer',season:state.season}}}}
function evolveClubProject(c,position,total){ensureClubProjects();const p=c?.careerProject;if(!p||!position)return;p.lastPosition=position;p.lastSeason=state.season;if(position<=Math.max(2,Math.floor(p.target*.55)))p.target=Math.max(2,p.target-1);else if(position>p.target+3)p.target=Math.min(Math.max(8,Math.ceil(total*.72)),p.target+1);p.level=p.target<=3?'Competir por títulos':p.target<=6?'Aspirar a playoff':'Construir y crecer'}
function projectObjectives(clubObj){const rep=clubObj?.reputation||65,target=clubObj?.careerProject?.target|| (rep>=86?3:rep>=76?6:rep>=66?9:12);return [{id:'LEAGUE',label:`Finalizar entre los ${target} primeros de liga`,target},{id:'YOUTH',label:'Desarrollar talento joven',target:rep<70?1:2},{id:'FIN',label:'Mantener una estructura financiera sostenible',target:1}]}
