...
S=session=requests.Session(impersonate="chrome")
...
def get_json(path, params=None, retries=3):
    ...
    for base in (PRIMARY,MIRROR):
        for attempt in range(retries):
            try:
                r=S.get(base+path,params=params,headers=HEADERS,timeout=25)
...
def search_team(cid):
    ...
    # Usually one query is enough; use alternate aliases only if confidence is low.
    for alias in aliases[:3]:
        ...
        for row in results:
            ...
            if sport_name(ent) not in ("basketball","basket"):
                continue
            name=ent.get("name") or ""
            n=norm(name)
            # Compare against every known alias, not only the query used.
            ratio=max(SequenceMatcher(None,n,norm(a)).ratio() for a in aliases)
            if n in {norm(a) for a in aliases}:ratio=1.0
            elif any(norm(a) in n or n in norm(a) for a in aliases):ratio=max(ratio,.90)
            if ratio>best_score and ent.get("id"):
                best_score=ratio;best={"id":ent["id"],"name":name,"score":ratio}
...
def team_players(team_id):
    data=get_json(f"/team/{team_id}/players")
    rows=data.get("players") or []
    out=[];seen=set()
    for row in rows:
        p=row.get("player") or row
        name=(p.get("name") or p.get("shortName") or "").strip()
        if not name or norm(name) in seen:continue
        seen.add(norm(name))
        item={"name":name,"pos":pos_text(p,row)}
        age=age_from_player(p)
        if age and 15<=age<=50:item["age"]=age
        country=p.get("country") or {}
        iso=country.get("alpha2") if isinstance(country,dict) else None
...
def main():
    ...
    matched_names={};errors=[]
    for cid in range(1,151):
        try:
            t=search_team(cid)
            if not t:
                errors.append(f"{cid:03d} {GAME_NAMES.get(cid,cid)}: team not confidently resolved")
                print(cid,"NO TEAM")
                continue
            roster=team_players(t["id"])
            if not roster:
                errors.append(f"{cid:03d} {GAME_NAMES.get(cid,cid)}: {t['name']} resolved but roster empty")
                print(cid,t['name'],"EMPTY")
                continue
            grouped[str(cid)]=roster
            matched_names[cid]=f"{t['name']} (Sofascore {t['id']})"
            print(cid,t['name'],len(roster))
        except Exception as e:
            errors.append(f"{cid:03d} {GAME_NAMES.get(cid,cid)}: {type(e).__name__}: {e}")
            print(cid,"ERROR",e)
        time.sleep(.16)

    payload={k:v for k,v in grouped.items() if v}
    covered=len(payload);players=sum(map(len,payload.values()))
    meta={
      "snapshot":SNAPSHOT,
      "source":"Sofascore current team roster endpoints",
      "coverage":{"clubs":covered,"totalClubs":150,"players":players},
      "sourceUrls":["https://www.sofascore.com/","https://api.sofascore.com/api/v1/team/{teamId}/players"]
    }
    ...
    for cid in range(1,151):
        n=len(grouped[str(cid)]);source=matched_names.get(cid,"—")
        lines.append(f"- {cid:03d} · {GAME_NAMES.get(cid,str(cid))}: **{n}** · {source}")
    if errors:
        lines += ["","## Unresolved / empty clubs",""]+[f"- {x}" for x in errors]
    REPORT.write_text("\n".join(lines)+"\n",encoding="utf-8")
    print(f"Generated {players} player identities across {covered}/150 clubs")
    # Do not silently publish a useless snapshot if the source/API changed.
    if covered<50 or players<500:
        raise SystemExit(f"Coverage gate failed: {covered} clubs / {players} players")
