...
function notificationActionsHtml(e){
  const actions=[];
  if(e.type==='TRANSFER_OFFER'&&!e.resolved)actions.push(`<button class="btn small good" data-offer-accept="${e.id}">Aceptar</button>`,`<button class="btn small" data-offer-counter="${e.id}">Renegociar</button>`,`<button class="btn small" data-offer-reject="${e.id}">Rechazar</button>`);
...
function resolveDecision(id,choiceIndex,fromModal=false){
  ...
  if(ch.effect==='BOARD_YOUTH'){state.board.prioritySeason=state.season;state.board.seasonPriority='YOUTH';result='La directiva evaluará la temporada con más margen si das muchos partidos a jugadores menores de 22 años. Si no desarrollas cantera y tampoco cumples objetivos, la confianza caerá con fuerza.'}
  if(ch.effect==='BOARD_RESULTS'){state.board.prioritySeason=state.season;state.board.seasonPriority='RESULTS';result='La directiva exigirá resultados esta temporada. No habrá penalización por no utilizar canteranos.'}
...
function applyMatchDecisionV48(m,res){
  ...
  const won=userHome?res.homeScore>res.awayScore:res.awayScore>res.homeScore;const tied=res.homeScore===res.awayScore;summary+=' '+(tied?'El partido terminó en empate.':won?'El equipo ganó el partido.':'El equipo perdió el partido.');const decisionResult={title:d.title,choice:d.choice,choiceLabel:d.choiceLabel||d.choice,summary,clock:d.clock||'0:08'};const history=Array.isArray(manager.history)?manager.history:(manager.history=[]);manager.pending=null;const historyEntry=[...history].reverse().find(x=>x&&x.matchId===m.id&&x.choice===d.choice&&!x.summary);if(historyEntry){historyEntry.summary=summary;historyEntry.result=decisionResult}res.matchDecision=decisionResult;manager.lastResult=res;saveLocal(false);
}

function playNextUserMatchV489(){
  try{return simulateToNextUserMatch()}
  catch(error){
    console.error('No se pudo simular el siguiente partido',error);
    ensureMatchDecisionStateV48();
    toast('No se pudo avanzar. La partida se ha conservado; vuelve a pulsar para reintentarlo.');
    saveLocal(false);
  }
}
...
function simulateToNextUserMatch(){
  ...
  const progressionDate=state.currentDate>nm.date?state.currentDate:nm.date;
  processAcademyTo(progressionDate);processScouting(progressionDate);processMedicalTo(progressionDate);
  state.currentDate=progressionDate;
...
function evaluateSeasonPriority(){const b=state.board||{},priority=b.prioritySeason===state.season?b.seasonPriority:null,objective=(b.objectives||[]).find(o=>o.id==='LEAGUE')||projectObjectives(userClub()).find(o=>o.id==='LEAGUE'),league=objective?.leagueId?sortedStandings(objective.leagueId):sortedStandings('ACB'),position=league.findIndex(x=>x.clubId===state.userClubId)+1,success=position>0&&position<=objective.target;if(priority!=='YOUTH')return {delta:0,text:priority==='RESULTS'?'La directiva no valora el uso de cantera por encima de los resultados.':''};const stats=seasonStatsMap(),youngGames=userClub().roster.filter(p=>(p.age||99)<22).reduce((n,p)=>n+(stats[p.id]?.g||0),0),developed=youngGames>=30;if(!success&&!developed)return {delta:-10,text:`La prioridad era cantera, pero solo acumulaste ${youngGames} partidos de jugadores sub-22 y no alcanzaste el objetivo deportivo.`};if(!success&&developed)return {delta:7,text:`Aunque no alcanzaste el objetivo deportivo, los ${youngGames} partidos de jugadores sub-22 dan margen a la directiva.`};return {delta:developed?3:0,text:developed?`${youngGames} partidos de jugadores sub-22 respaldan la prioridad de cantera.`:''}}
function evaluateCoachRequestSeason(){const req=state.coachManagement?.squadRequest;if(!req||req.season!==state.season||req.status!=='ACCEPTED'||req.evaluated)return;const signed=Array.isArray(req.baselinePlayerIds)&&userClub().roster.some(p=>p.primaryPosition===req.position&&!req.baselinePlayerIds.includes(p.id)),delta=signed?7:-14;state.coachManagement.relationship=BBGM.clamp((state.coachManagement.relationship||72)+delta,0,100);req.evaluated=true;req.fulfilled=signed;addInbox('COACH',signed?'Has cumplido la petición del entrenador':'Compromiso incumplido con el entrenador',signed?`El refuerzo de ${positionLabel[req.position]} llega como acordasteis. Vuestra relación mejora notablemente.`:`Aceptaste reforzar ${positionLabel[req.position]}, pero no lo hiciste. Vuestra relación baja mucho.`,{coachRequestSeason:state.season})}
...
function startNextSeason(){
  ...
  ensureSeasonBoardPriority();addInbox('SPONSOR','Nuevas ofertas de patrocinio',`Ya puedes elegir patrocinador para ${state.season}.`);
...
function counterIncomingOffer(id){const ev=state.inbox.find(e=>e.id===id);if(!ev||ev.resolved)return;const buyer=club(ev.fromClubId),p=userClub().roster.find(x=>x.id===ev.playerId);if(!buyer||!p)return;const floor=Math.max(Math.round(ev.fee*1.05/50000)*50000,Math.round(BBGM.marketValue(p)*.95/50000)*50000),raw=prompt(`Contraoferta por ${fullName(p)}\nOferta actual: ${fmtMoney(ev.fee)}\nMínimo razonable: ${fmtMoney(floor)}`,String(Math.round(ev.fee*1.18/50000)*50000));if(raw==null)return;const fee=Math.round(Number(raw)/50000)*50000;if(!Number.isFinite(fee)||fee<floor){toast(`La contraoferta debe ser de al menos ${fmtMoney(floor)}`);return}const ceiling=Math.min(buyer.cashBudget,Math.round(Math.max(ev.fee*1.12,BBGM.marketValue(p)*1.35)/50000)*50000),rng=careerRngV4813(`counter-${ev.id}-${fee}`);if(fee<=ceiling&&rng.next()<.78){ev.fee=fee;ev.countered=true;ev.decisionResult=`${buyer.name} acepta tu contraoferta de ${fmtMoney(fee)}. Ahora puedes aceptar o rechazar el traspaso.`;saveLocal(false);render();toast('Contraoferta aceptada por el club')}else{ev.resolved=true;ev.decision='COUNTER_REJECTED';ev.decisionResult=`${buyer.name} rechaza la contraoferta y retira su propuesta.`;saveLocal(false);render();toast('El club rechaza la contraoferta')}}
...
function bindInboxActions(root=document){
  ...
  root.querySelectorAll('[data-offer-counter]').forEach(b=>b.onclick=()=>counterIncomingOffer(+b.dataset.offerCounter));
...
function renderHome(v){
  ...
  const nextBtn=document.getElementById('nextMatchBtn');if(nextBtn)nextBtn.onclick=pending?()=>interruptForPendingDecision():state.seasonComplete?showEndSeasonModal:state.offseason?.active?advanceOffseasonWeek:state.preseason?.active?advancePreseasonWeek:nextScoutingCompletion()&&(!nm||nextScoutingCompletion().endDate<nm.date)?advanceToNextEvent:nm?playNextUserMatchV489:advanceToNextEvent;if(nm)document.getElementById('playBtn').onclick=playNextUserMatchV489;const quick=document.getElementById('v45NextStep');if(quick&&nextBtn)quick.onclick=()=>nextBtn.click();
...
function renderOffers(root){
  ...
  root.innerHTML=`<div class="card"><h3>Ofertas por tus jugadores</h3>${offers.length?offers.map(e=>{const loc=playerLocation(e.playerId),buyer=club(e.fromClubId),status=e.decision==='ACCEPTED'?'Aceptada':e.decision==='COUNTER_REJECTED'?'Contraoferta rechazada':'Rechazada';return `<div class="offer-card ${e.resolved?'resolved':''}"><div><b>${loc?fullName(loc.player):'Jugador transferido'}</b><div class="muted">${buyer?buyer.name:'Club'} · ${fmtMoney(e.fee)}${e.countered?' · Contraoferta aceptada':''}</div></div><div>${e.resolved?`<span class="pill">${status}</span>`:`<button class="btn small good" data-offer-accept="${e.id}">Aceptar</button> <button class="btn small" data-offer-counter="${e.id}">Renegociar</button> <button class="btn small" data-offer-reject="${e.id}">Rechazar</button>`}</div></div>`}).join(''):'<p class="muted">Todavía no has recibido ofertas.</p>'}</div>${state.marketNews.length?`<div class="card" style="margin-top:16px"><h3>Movimientos del mercado</h3>${state.marketNews.slice(0,12).map(n=>`<div class="news-line"><span class="muted">${n.date}</span><span>${n.text}</span></div>`).join('')}</div>`:''}`;
