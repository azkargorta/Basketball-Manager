...
const BBGM=g.BBGM;
const APP_VERSION=g.BBGM_VERSION||{code:'0.31.0-beta',label:'v0.31 Beta',saveFormat:'basketball-manager-v031'};
...
function aiPositionNeed(c,pos){const d=c.roster.filter(p=>p.primaryPosition===pos||p.secondaryPosition===pos).sort((a,b)=>BBGM.overall(b,pos)-BBGM.overall(a,pos));const top=d[0]?BBGM.overall(d[0],pos):0,sec=d[1]?BBGM.overall(d[1],pos):0;return (d.length<2?35:0)+Math.max(0,78-top)*1.7+Math.max(0,70-sec)}
function aiFitScore(c,p){const pos=Math.max(aiPositionNeed(c,p.primaryPosition),p.secondaryPosition?aiPositionNeed(c,p.secondaryPosition):0),age=(p.age||27),profile=c.reputation>=90?(BBGM.overall(p)>=80?15:-5):c.reputation>=78?(age<=28?8:2):(age<=24?12:0);return pos+profile+BBGM.overall(p)*.3-(p.salary||0)/1000000*.3}
...
function advancePreseasonWeek(){ensureV17State();if(!state.preseason?.active)return;const fr=state.preseason.friendlies.find(x=>x.status==='SCHEDULED');if(fr){simulatePreseasonFriendly(fr);state.currentDate=fr.date;addInbox('RESULT',`Amistoso: ${userClub().shortName} ${fr.home?fr.homeScore:fr.awayScore}-${fr.home?fr.awayScore:fr.homeScore} ${club(fr.opponentId).shortName}`,`Pretemporada · ${fr.date}`)}applyPreseasonFocus();processScouting(state.currentDate);processMedicalTo(state.currentDate);state.preseason.weeksRemaining--;maybeRecordWeeklySummary(true);if(state.preseason.weeksRemaining<=0){state.preseason.active=false;state.currentDate=addDays(state.currentDate,2);state.preseasonReviews.unshift({season:state.season,focus:state.preseason.focus,results:state.preseason.friendlies.map(x=>({opponentId:x.opponentId,homeScore:x.homeScore,awayScore:x.awayScore,home:x.home}))});addInbox('SEASON','Pretemporada completada','La plantilla queda lista para la competición oficial. Revisa los últimos roles y el estado físico antes del primer partido.');toast('Pretemporada completada')}else toast(`Quedan ${state.preseason.weeksRemaining} semanas de pretemporada`);saveLocal(false);render()}
...
function newGame(){
  ...
  state={
    version:APP_VERSION.code,saveName:'Carrera Baskonia',season:'2026/27',currentDate:'2026-09-01',userClubId:1,nextEventId:100,
  ...
  saveLocal(false);currentView='home';render();
}
...
function runAiMarketStep(){
  ...
  if(rng.next()<.42){
    ...
    if(!existing&&uc.roster.length>9){
      ...
      if(candidates.length){
        ...
        const buyers=state.world.clubs.filter(c=>c.id!==uc.id&&c.cashBudget>BBGM.marketValue(p)*.75&&(c.salaryBudget-BBGM.wageBill(c))>p.salary*1.08).sort((a,b)=>aiFitScore(b,p)-aiFitScore(a,p));
        if(buyers.length){
          const buyer=buyers[Math.floor(rng.next()*Math.min(4,buyers.length))],factor=(p.transferListed?1.02:.90)+rng.next()*.32;
          const fee=Math.round(BBGM.marketValue(p)*factor/50000)*50000;
  ...
  if(rng.next()<.16){
    const sellers=state.world.clubs.filter(c=>c.id!==uc.id&&c.leagueLevel!=='NBA'&&c.roster.length>=12);if(sellers.length){const seller=rng.pick(sellers);const surplus=seller.roster.slice().sort((a,b)=>aiFitScore(seller,a)-aiFitScore(seller,b))[0];if(surplus){const buyers=state.world.clubs.filter(c=>c.id!==uc.id&&c.id!==seller.id&&c.leagueLevel!=='NBA'&&c.cashBudget>BBGM.marketValue(surplus)*.7&&(c.salaryBudget-BBGM.wageBill(c))>surplus.salary).sort((a,b)=>aiFitScore(b,surplus)-aiFitScore(a,surplus));const buyer=buyers[0];if(buyer&&aiFitScore(buyer,surplus)>aiFitScore(seller,surplus)+8){const fee=Math.round(BBGM.marketValue(surplus)*(.78+rng.next()*.22)/50000)*50000;seller.roster=seller.roster.filter(x=>x.id!==surplus.id);buyer.roster.push(surplus);seller.cashBudget+=fee;buyer.cashBudget-=fee;surplus.state.teamAdaptation=48;state.marketNews.unshift({date:state.currentDate,text:`${buyer.name} ficha a ${fullName(surplus)} desde ${seller.name} por ${fmtMoney(fee)}.`})}}}
  }
}
...
function navButton(view,label,icon){return `<button data-view="${view}" class="${currentView===view?'active':''}">${icon} ${label}</button>`}
...
function inboxHtml(){
  ...
  return items.map(x=>{
    const actions=x.type==='TRANSFER_OFFER'?`<div class="inbox-actions"><button class="btn small good" data-offer-accept="${x.id}">Aceptar</button><button class="btn small" data-offer-reject="${x.id}">Rechazar</button></div>`:x.type==='DECISION'&&!x.resolved?`<div class="inbox-actions">${(x.choices||[]).map((c,i)=>`<button class="btn small ${i===0?'good':''}" data-decision="${x.id}" data-choice="${i}">${c.label}</button>`).join('')}</div>`:'';
    return `<div class="inbox-item"><span class="dot ${x.type==='INJURY'?'bad':x.type==='TRAINING'||x.type==='CONTRACT'?'warn':''}"></span><div class="inbox-body"><div class="msg-title">${x.title}</div><div class="msg-text">${x.text}</div>${actions}</div></div>`;
  }).join('');
}
...
function renderHome(v){
  const nm=nextUserMatch(),uc=userClub();
...
function renderSchedule(v){
  ...
  v.innerHTML=`<div class="section-title"><div><div class="eyebrow">Temporada</div><h1>Calendario y agenda</h1><p>${list.filter(x=>x.status==='PLAYED').length} de ${list.length} partidos oficiales disputados.</p></div><button class="btn primary" id="simNext" ${nextUserMatch()?'':'disabled'}>Siguiente partido</button></div><div class="calendar-toolbar"><div class="tabs"><button data-schedule-mode="month" class="${scheduleMode==='month'?'active':''}">Mes</button><button data-schedule-mode="list" class="${scheduleMode==='list'?'active':''}">Lista</button></div><select id="scheduleMonth">${months.map(m=>`<option value="${m}" ${m===scheduleMonth?'selected':''}>${new Intl.DateTimeFormat('es-ES',{month:'long',year:'numeric'}).format(new Date(m+'-01T12:00:00'))}</option>`).join('')}</select></div><div class="grid schedule-layout"><div class="card">${scheduleMode==='month'?monthCalendarHtml(scheduleMonth):listHtml}</div>${contractAgendaHtml()}</div>`;
...
function renderMore(v){
  ...
  v.innerHTML=`<div class="section-title"><div><div class="eyebrow">Partida</div><h1>Más</h1><p>Directiva, entrenador, patrocinadores, guardado y carrera.</p></div></div><div class="grid two">
...
function finalizeContract(p,context,offer){
  ...
  addInbox('CONTRACT',context.type==='RENEW'?'Renovación completada':'Fichaje completado',`${fullName(p)} firma ${offer.years} año(s) por ${fmtMoney(offer.salary)} anuales.`);
}

function showResultModal(m,res){
  if(!res)return;const h=club(m.homeClubId),a=club(m.awayClubId);
  ...
  const back=modal(`<div class="modal-head"><div><div class="eyebrow">${comp(m.competitionId).name}</div><b>${typeof m.round==='number'?'Jornada '+m.round:m.round} · ${m.date}</b></div><button class="btn" data-close>Cerrar</button></div><div class="result-head"><div class="result-teams">${h.name} — ${a.name}</div><div class="result-score">${res.homeScore} - ${res.awayScore}</div><div class="muted">${res.overtimePeriods?res.overtimePeriods+' prórroga(s) · ':''}${Math.round(res.homeTeamStats.possessions)} posesiones</div></div><div class="tabs"><button class="active" data-tab="home">${h.shortName}</button><button data-tab="away">${a.shortName}</button></div><div id="boxContent">${table(h,res.homeStats)}</div>`);
  back.querySelector('[data-close]').onclick=()=>back.remove();back.querySelectorAll('[data-tab]').forEach(b=>b.onclick=()=>{back.querySelectorAll('[data-tab]').forEach(x=>x.classList.remove('active'));b.classList.add('active');back.querySelector('#boxContent').innerHTML=b.dataset.tab==='home'?table(h,res.homeStats):table(a,res.awayStats)});
}
