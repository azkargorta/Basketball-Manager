...
function renderHome(v){
  ...
  v.insertAdjacentHTML('afterbegin',`<div class="v45-priority-strip"><button type="button" id="v45NextStep" class="v45-priority-card ${pending?'urgent':''}"><div><b>${pending?'Decisión pendiente':'Siguiente paso'}</b><span>${pending?'Resuélvela antes de simular':'Avanza la carrera desde aquí'}</span></div><strong>${pending?'!':'→'}</strong></div><div class="v45-priority-card"><div><b>Directiva</b><span>${Math.round(state.board?.confidence??70)}/100 de confianza</span></div><strong>${Math.round(state.board?.confidence??70)}</strong></div><div class="v45-priority-card"><div><b>Próximo partido</b><span>${nm?`${club(nm.homeClubId===state.userClubId?nm.awayClubId:nm.homeClubId)?.shortName||'Rival'} · ${nm.date}`:'Sin partido programado'}</span></div><strong>${nm?'▸':'—'}</strong></div></div>`);
