...
/*
  ...
  Private friends-only pack. No real photos or logos.
  Only initial real-world factual identity data belongs here.
  Ratings, salaries, personality, morale and future events remain simulation data.
...
const REAL_PACK={
  ...
  sources:['ACB.com · plantillas y tabla de mercado 2026-27 · snapshot 2026-09-02'],
  clubs:{
    1:[
      {name:'Damion Baugh',position:'PG'},
      {name:'Kobi Simmons',position:'PG'},
      {name:'Markus Howard',position:'SG'},
      {name:'Matteo Spagnolo',position:'SG'},
      {name:'AJ Lawson',position:'SF'},
      {name:'Chris Duarte',position:'SF'},
      {name:'Dewayne Stewart Jr',position:'SF'},
      {name:'Rodions Kurucs',position:'SF'},
      {name:'Stefan Joksimovic',position:'SF'},
      {name:'Tadas Sedekerskis',position:'SF'},
      {name:'Clement Frisch',position:'PF'},
      {name:'Kenneth Faried',position:'PF'}
    ],
    2:[
    ...
    3:[
      ...
      {name:'Justin Minaya',position:'SF'},
      {name:'Joel Parra',position:'SF',nationality:'ESP',age:26,height:202},
    ...
    4:[
      ...
      {name:'Elias Valtonen',position:'SF'},
      {name:'Dylan Osetkowski',position:'PF',nationality:'GER',age:30,height:206},
      {name:'Nate Reuvers',position:'PF',nationality:'HUN',age:27,height:211},
      {name:'Mo Gueye',position:'PF',nationality:'SEN',age:28,height:206},
      {name:'Jasiel Rivero',position:'PF'},
...
function applyClubRoster(club,rows){
  if(!club||!Array.isArray(rows)||rows.length<10)return;
  while(club.roster.length<rows.length){
    const clone=Object.assign({},club.roster[club.roster.length-1]);
    clone.attributes=Object.assign({},clone.attributes||{});
    clone.tendencies=Object.assign({},clone.tendencies||{});
    clone.id=(club.id*100000)+club.roster.length+1;
    club.roster.push(clone);
  }
  if(club.roster.length>rows.length)club.roster.length=rows.length;
  for(let i=0;i<rows.length;i++)applyIdentity(club.roster[i],rows[i]);
  club.realWorldRoster=true;
  club.realWorldSnapshot='2026-09-02';
  club.realWorldRosterCount=rows.length;
}
function applyRealPack(world){
  if(!world?.clubs)return world;
  let realClubs=0,realPlayers=0;
  for(const c of world.clubs){
    const rows=REAL_PACK.clubs[c.id];
    if(rows){applyClubRoster(c,rows);realClubs++;realPlayers+=rows.length;}
  }
  world.dataPack={
    id:REAL_PACK.id,label:REAL_PACK.label,version:REAL_PACK.version,
    privateUse:true,provisional:REAL_PACK.provisional,
    realClubs,realPlayers,totalClubs:world.clubs.length
  };
  return world;
}
