...
const REAL_PACK={id:'real_2026_27',version:g.BBGM_VERSION?.dataPackVersion||'2026-09-03b',privateUse:true,provisional:true,label:'Real 2026/27 · beta',notes:'Identidades reales iniciales; ratings, potenciales y economía son estimaciones originales del juego.',clubs:Object.assign({},FALLBACK_CLUBS,GENERATED,MANUAL)};
...
function applyIdentity(player,row){const n=splitName(row.name);player.firstName=n.firstName;player.lastName=n.lastName;if(row.nationality)player.nationality=row.nationality;if(Number.isFinite(row.age))player.age=row.age;const pos=normalizePosition(row.position);if(pos)player.primaryPosition=pos;const sec=normalizePosition(row.secondaryPosition);if(sec)player.secondaryPosition=sec;if(typeof BBGM.normalizeRealWorldPlayer==='function')BBGM.normalizeRealWorldPlayer(player,row);else{if(Number.isFinite(row.height))player.heightCm=row.height;setTargetOverall(player,Number(row.ovr))}if(Number.isFinite(row.potential))player.potentialReal=Math.max(Number(row.potential),Number(row.ovr)||0);else if(Number.isFinite(row.ovr))player.potentialReal=Math.max(Number(player.potentialReal)||0,Number(row.ovr));player.realWorldIdentity=true;player.realWorldSnapshot='2026-09-03';player.salaryIsGameEstimate=true;player.personalitySimulationOnly=true}
...
function applyRealPack(world){if(!world?.clubs)return world;let realClubs=0,realPlayers=0;for(const c of world.clubs){const rows=REAL_PACK.clubs[c.id];if(rows&&applyClubRoster(c,rows)){realClubs++;realPlayers+=rows.length}}const normalized=world.clubs.flatMap(c=>c.roster||[]).filter(p=>p.realWorldIdentity);const quality=BBGM.realWorldDataQuality?.summarize?.(normalized)||null;world.dataPack={id:REAL_PACK.id,label:REAL_PACK.label,version:REAL_PACK.version,privateUse:true,provisional:true,realClubs,realPlayers,totalClubs:world.clubs.length,generatedMeta:GENERATED_META,quality};return world}
